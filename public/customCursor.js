...
function handleInitialMouseMove(e) {
    if (!hasMouseMoved) {
        // Only run this logic once on the very first mouse move
        cursor.style.opacity = '1';    // Make cursor visible
        follower.style.opacity = '1'; // Make follower visible
        hasMouseMoved = true;          // Set the flag

        // You might want a slightly delayed hide/show to keep opacity at 1.
        // If your CSS active class handles opacity, you can simplify:
        cursor.classList.add('visible'); // Add a new class to control initial visibility
        follower.classList.add('visible'); // Add a new class to control initial visibility
